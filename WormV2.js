...
export async function main(ns) {
     ...
     * Main
     */
    await addServersToObject(currentServerName);
        ...
        for(var x in servers){
            for(var y in servers[x].connectedServers){
                ...
                if(await compareArray(ns, serverNames, servers[x].connectedServers[y]) == false){
                    ...
                    await addServersToObject(servers[x].connectedServers[y]);
    ...
    async function addServersToObject(currentServerName1){
        var currentServerInformationFileName = currentServerName1 + serverInformationFileName;
        var currentServerConnectedServersFileName = currentServerName1 + connectedServersFileName;
        var serverInfo = await canImportData(currentServerName1, currentServerInformationFileName);
        var connectedServers = await canImportData(currentServerName1, currentServerConnectedServersFileName);
